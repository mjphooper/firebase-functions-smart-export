...
export async function main() {
    try {
        ...
        if (verbose) {
            styledConsoleOutput.info(config ? 'Config file loaded.' : 'No "ffse.config.json" file found.');
        }
        ...
        if (dryRun) {
            styledConsoleOutput.success(`✅ Dry run complete! ${registrySize} function(s) found to export in ${files.length} file(s).${verbose && !isRegistryEmpty ? ' To see the full list of exported functions, run with "--verbose".' : ''}`);
            process.exit(0);
        }
        if (isRegistryEmpty) {
        ...
        const indexFile = new Project().createSourceFile(await getAbsGeneratedIndexPath(), undefined, { scriptKind: ScriptKind.JS, overwrite: true });
        generateRegistryFile('', registry);
        if (verbose)
            styledConsoleOutput.info(`Generated "${REGISTRY_FILE_NAME}".`);
        await generateIndexFile(indexFile, registry, config);
        if (verbose)
            styledConsoleOutput.info(`Generated "${GENERATED_INDEX_FILE_NAME}".`);
