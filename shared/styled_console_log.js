 ...
 */
function outputWithStyle(message, chalkColor) {
...
export const styledConsoleOutput = {
    ...
    info: (message) => {
        if (!runContext.isTest)
            console.log(outputWithStyle(`INFO: ${message}`, chalk.blue));
    },
    warn: (message) => {
        if (!runContext.isTest)
            console.warn(outputWithStyle(`WARNING: ${message}`, chalk.yellow));
    },
    error: (message) => {
        if (!runContext.isTest)
            console.error(outputWithStyle(`ERROR: ${message}`, chalk.red));
    },
    success: (message) => {
        if (!runContext.isTest)
            console.log(outputWithStyle(`${message}`, chalk.green));
    },
};
